...
This preview imports `AgreeFirst` from the published package. Check the box, inspect the record, then open **Code** to see the exact React component running here. The callback keeps data in page memory only; in your app, replace that with your own form or backend save function.

<LiveExample title="Simple agreement" code={simpleAgreementCode}>
  <SimpleAgreementExample client:load />
</LiveExample>
